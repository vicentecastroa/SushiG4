...
class InventoryWorker < InventoriesController
	...
	def mover_ingrediente_a_despacho(sku, stock_minimo)
		...
		if stock_en_almacen[@@id_recepcion]["cantidad"] >= unidades_por_mover
			#TODO mover_a_almacen(@@id_recepcion, @@id_despacho, [sku], unidades_por_mover)
		else 
			#TODO mover_a_almacen(@@id_recepcion, @@id_despacho, [sku], 0)
			unidades_por_mover -= stock_en_almacen[@@id_recepcion]["cantidad"]
		end

		if stock_en_almacen[@@id_pulmon]["cantidad"] >= unidades_por_mover
			#TODO mover_a_almacen(@@id_pulmon, @@id_despacho, [sku], unidades_por_mover)
		else 
			#TODO mover_a_almacen(@@id_pulmon, @@id_despacho, [sku], 0)
			unidades_por_mover -= stock_en_almacen[@@id_pulmon]["cantidad"]
		end

		if stock_en_almacen[@@id_cocina]["cantidad"] >= unidades_por_mover
			#TODO mover_a_almacen(@@id_cocina, @@id_despacho, [sku], unidades_por_mover)
		else 
			#TODO mover_a_almacen(@@id_cocina, @@id_despacho, [sku], 0)
